...
Cada CSV se asume de 1 día y contiene líneas separadas por ';' con campos:
  L1: timestamp20;level;type;price;volume
  L2: timestamp20;level;type;price;volume;action;depth  (se ignora por level != 1)

Salida (por contrato y tipo de dato):
  ES 09-24.Last.txt
  ES 09-24.Bid.txt
  ES 09-24.Ask.txt
  ES 09-24.All.txt   <-- Tick Replay Format (last;bid;ask;volume)

Formato por línea (Last/Bid/Ask):
  yyyyMMdd HHmmss ffffffff;price;volume

Formato por línea (All / Tick Replay):
  yyyyMMdd HHmmss ffffffff;last;bid;ask;volume

Donde ffffffff tiene 7 dígitos. Si el origen trae microsegundos (6 dígitos), se agrega '0'.
"""
...
def ts20_to_nt_parts(ts20: str) -> Tuple[str, str, str]:
    ...
    Convierte 'YYYYMMDDhhmmssffffff' (20 dígitos) a:
        fecha -> 'yyyyMMdd'
        hora  -> 'HHmmss'
        frac7 -> 'fffffff' (7 dígitos)
    Tomamos los microsegundos (6) y agregamos '0' al final.
    """
    fecha: str = ts20[:8]     # YYYYMMDD
    hora: str = ts20[8:14]    # hhmmss
    micros6: str = ts20[14:20]  # ffffff
    frac7: str = micros6 + "0"
...
def second_friday(year: int, month: int) -> date:
    """Fecha del segundo viernes del mes dado."""
...
def front_contract_for_date(d: date) -> Tuple[int, int]:
    """
    Dada una fecha de trading 'd', retorna (mes_contrato, año_contrato) del front
    ...
    if d < r_mar:
        ...
        # Luego del rollover de diciembre, el front pasa a marzo del año siguiente
        return 3, d.year + 1
...
def infer_symbol_from_path(csv_path: Path, forced_symbol: Optional[str]) -> str:
    """
    Intenta obtener el símbolo desde la carpeta 'ES_T2_YYYYMM'.
    Si no puede, usa --symbol o lanza error.
    """
    if forced_symbol:
        return forced_symbol

    # Buscar en carpeta y carpeta padre
    ...
    raise ValueError(
        f"No se pudo inferir el símbolo desde '{csv_path}'. "
        f"Usa --symbol para indicarlo manualmente."
    )


def infer_trade_date_from_filename(csv_path: Path) -> date:
    """
    Extrae la fecha (YYYYMMDD) desde 'YYYYMMDD.csv'.
    """
...
class ContractWriters:
    ...
    También mantiene el estado (últimos valores conocidos) para emitir .All.
    """
    def __init__(self) -> None:
        self.handles: Dict[Tuple[ContractKey, str], TextIO] = {}
        # Estado por contrato para Tick Replay (.All)
        # {'last': Optional[str], 'bid': Optional[str], 'ask': Optional[str]}
...
def export_csv_into_contract_files(
    ...
    Retorna contadores:
      (total_lineas, lineas_validas, l1_total, l1_bids, l1_asks, l1_trades, all_emits)
    """
    ...
    with csv_path.open("r", newline="", encoding="utf-8") as fin:
        for campos in iter_csv_lineas(fin):
            ...
            if tipo == TYPE_LAST:
                ...
                l1_trades += 1
                # Actualiza estado .All
                st["last"] = price_str
                # Emitir .All si ya tenemos bid y ask
                if st["bid"] is not None and st["ask"] is not None:
                    escribir_all(f"{fecha} {hora} {frac7};{st['last']};{st['bid']};{st['ask']};{volume_str}\n")
                    all_emits += 1
                ...
                # Actualiza estado .All
                st["bid"] = price_str
                # Emitir .All si ya tenemos last y ask (volume=0 para quotes)
                if st["last"] is not None and st["ask"] is not None:
                    escribir_all(f"{fecha} {hora} {frac7};{st['last']};{st['bid']};{st['ask']};0\n")
                    all_emits += 1
                ...
                # Actualiza estado .All
                st["ask"] = price_str
                # Emitir .All si ya tenemos last y bid (volume=0 para quotes)
                if st["last"] is not None and st["bid"] is not None:
                    escribir_all(f"{fecha} {hora} {frac7};{st['last']};{st['bid']};{st['ask']};0\n")
                    all_emits += 1
    ...
    print(
        ...
        f"L1={l1_total}, bids={l1_bids}, asks={l1_asks}, trades={l1_trades}, all={all_emits}"
    )

    return total_lineas, lineas_validas, l1_total, l1_bids, l1_asks, l1_trades, all_emits
...
def main() -> None:
    ...
    parser.add_argument(
        "--no-recursive", dest="no_recursive", action="store_true",
        help="No buscar recursivamente (*.csv). Por defecto busca recursivo."
    )
    parser.add_argument(
        "--symbol", dest="symbol", default=None,
    ...
    with ContractWriters() as writers:
        for csv_path in csv_paths:
            try:
                total_archivos += 1
                c_lineas, c_validas, c_l1, c_bids, c_asks, c_trades, c_all = export_csv_into_contract_files(
                    csv_path=csv_path,
                    writers=writers,
                    out_dir=ruta_out,
