...
Salidas por contrato:
  <SYMBOL> MM-YY.Last.txt   -> "yyyyMMdd HHmmss fffffff;price;volume"
  <SYMBOL> MM-YY.Bid.txt    -> idem
  <SYMBOL> MM-YY.Ask.txt    -> idem
  <SYMBOL> MM-YY.All.txt    -> "yyyyMMdd HHmmss fffffff;last;bid;ask;volume"
                               **Solo emite en ticks Last (trades). No emite en Bid/Ask.**
"""
...
def export_csv_into_contract_files(
    ...
    l1_bids = l1_asks = l1_trades = all_emits = 0
    ...
    with csv_path.open("r", newline="", encoding="utf-8") as fin:
        for campos in iter_csv_lineas(fin):
            ...
            if tipo == TYPE_LAST:
                ...
                # Estado para .All
                st["last"] = price_str
                # Emitir SOLO en trades (Last) y solo si ya tenemos Bid y Ask previos
...
def main() -> None:
    ...
    total_l1 = total_bids = total_asks = total_trades = total_all = 0
