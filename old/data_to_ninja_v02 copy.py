...
Formato por línea para NinjaTrader (Tick sub-segundo):
...
# =========================
# Escritores por contrato
# =========================

class ContractWriters:
    """
    Mantiene abiertos (append) archivos Last/Bid/Ask por contrato durante toda la corrida.
    """
    ...
    def get_writers(self, ck: ContractKey, out_dir: Path) -> Tuple[TextIO, TextIO, TextIO]:
        ...
        if key_last not in self.handles:
            out_dir.mkdir(parents=True, exist_ok=True)
            self.handles[key_last] = ck.outfile(out_dir, "Last").open(
                "a", newline="", encoding="utf-8"
            )
        if key_bid not in self.handles:
            self.handles[key_bid] = ck.outfile(out_dir, "Bid").open(
                "a", newline="", encoding="utf-8"
            )
        if key_ask not in self.handles:
            self.handles[key_ask] = ck.outfile(out_dir, "Ask").open(
                "a", newline="", encoding="utf-8"
            )

        return self.handles[key_last], self.handles[key_bid], self.handles[key_ask]
...
def export_csv_into_contract_files(
    ...
    print(
        ...
        f"(Last/Bid/Ask) | líneas={total_lineas}, válidas={lineas_validas}, "
        f"L1={l1_total}, bids={l1_bids}, asks={l1_asks}, trades={l1_trades}"
    )

    return total_lineas, lineas_validas, l1_total, l1_bids, l1_asks, l1_trades
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(segundo viernes de Mar/Jun/Sep/Dic). Genera 3 archivos por contrato: "
            "'<SYMBOL> MM-YY.Last.txt', '.Bid.txt', '.Ask.txt'."
        )
    )
    parser.add_argument(
    ...
    with ContractWriters() as writers:
        for csv_path in csv_paths:
            try:
                total_archivos += 1
                c_lineas, c_validas, c_l1, c_bids, c_asks, c_trades = export_csv_into_contract_files(
                    csv_path=csv_path,
                    writers=writers,
                    out_dir=ruta_out,
